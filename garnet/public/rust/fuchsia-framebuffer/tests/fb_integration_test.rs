...
use fuchsia_framebuffer::{FrameBuffer, FrameUsage, PixelFormat, VSyncMessage};
...
fn test_main() -> Result<(), Error> {
    ...
    executor.run_singlethreaded(async {
        ...
        let mut fb = FrameBuffer::new(2, FrameUsage::Cpu, None, Some(sender))
            .await
        ...
        let image_ids = fb.get_image_ids().iter().map(|a| *a).collect::<Vec<u64>>();
        let frame_id_1 = image_ids[0];
        let frame_id_2 = image_ids[1];

        let frame = fb.get_frame_mut(frame_id_1);
        ...
        fb.present_frame(frame_id_1, Some(image_sender))?;

        let frame2 = fb.get_frame_mut(frame_id_2);
        ...
        fasync::spawn_local(async move {
            receiver.next().await;
            fb.present_frame(frame_id_2, None).expect("frame2 present to succeed");
